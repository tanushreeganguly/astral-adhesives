...
if($title){
	$select     = $objTypes->select("tbl_homebanner", "*", "is_delete = :is_delete AND title1 LIKE :title1 OR title2 LIKE :title2", $where, "id DESC");
}
else{
	$select     = $objTypes->select("tbl_homebanner", "*", "is_delete = :is_delete", $where, "id DESC");
}
...
if($title){
    $TypeArray = $objTypes->select("tbl_homebanner", "*", "is_delete = :is_delete AND title1 LIKE :title1 OR title2 LIKE :title2", $where, "id DESC", $limit);
}
else{
    $TypeArray = $objTypes->select("tbl_homebanner", "*", "is_delete = :is_delete", $where, "id DESC", $limit);
}
?>
...
<div class="content-wrapper">
  ...
  <section class="content-header">
    <h1> List Home Banner <small></small> </h1>
    <ol class="breadcrumb">
      <li><a href="add_homebanner.php"><i class="fa fa-plus-square"></i> Add Home Banner</a></li>
    </ol>
  </section>
  <!-- Main content -->
            ...
            <table id="example2" class="table table-bordered table-hover" >
			  ...
			  ?>
                <tr>
                  ...
                  if($val['image'] != ""){
                      ?>
                      <img src="../uploads/homebanner_images/large/<?php echo $val['image']; ?>" width="100px"  />
                      <?php
                  }
                  ?></td>
				   <td><?php
                  if($val['image1'] != ""){
                      ?>
                      <img src="../uploads/homebanner_images/medium/<?php echo $val['image1']; ?>" width="100px"  />
                      <?php
                  }
                  ?></td>
				  <td><?php
                  if($val['image2'] != ""){
                      ?>
                      <img src="../uploads/homebanner_images/small/<?php echo $val['image2']; ?>" width="100px"  />
                      <?php
                  }
                  ?></td>
                  <td><a href="act_homebanner.php?id=<?=$val['id']?>&status=<?=$val['is_active']?>&pgNo=<?=base64_encode($pgNo)?>">
                    <?=($val['is_active'] == "1") ? "<span class='label label-success'>Active</span>":"<span class='label label-danger'>Inactive</span>" ?>
                    </a> </td>
                  <td>
                  <a href="add_homebanner.php?id=<?=$val['id']?>&pgNo=<?=base64_encode($pgNo)?>"><i class="fa  fa-edit"></i></a>&nbsp;&nbsp;&nbsp;
                  <a href="act_homebanner.php?id=<?=$val['id']?>&pgNo=<?=base64_encode($pgNo)?>&action=delete" onclick="return window.confirm('Do you want to delete this record?')"><i class="fa  fa-trash"></i></a>
                  </td>
                </tr>
...
function Delete_CheckBox()
{
	...
	if(numberOfChecked>=1)
	{
		if(confirm("Are you sure  want to Delete Selected Record ?."))
		{
			...
			document.frmListing.action='act_homebanner.php?action=deleteall';
			document.frmListing.submit();
		}
	}
	else
...
function Active_CheckBox()
{
	...
	if(numberOfChecked>=1)
	{
		if(confirm("Are you sure  want to Activate Selected Record ?."))
		{
			...
			document.frmListing.action='act_homebanner.php';
			document.frmListing.submit();
		}
	}
	else
